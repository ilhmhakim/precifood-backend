...
export class MenuValidation {
    static readonly GETALLMENU: ZodType = z.object({
        ...
        role: z.enum(['Konsumen', 'Restoran', 'Admin'], {
            errorMap: () => ({
                message:
                    "Role harus bernilai 'Konsumen', 'Restoran', atau 'Admin'",
            }),
        }),
    });
    ...
    static readonly CREATEMENUNUTRITION = z.object({
        ...
        sfa: z
            ...
            .refine((val) => /^\d+(\.\d{1,2})?$/.test(val.toString()), {
                message:
                    'SFA harus memiliki maksimal 1 atau 2 angka di belakang koma',
            }),
        mufa: z
            .number()
            .positive()
            .refine((val) => /^\d+(\.\d{1,2})?$/.test(val.toString()), {
                message:
                    'MUFA harus memiliki maksimal 1 atau 2 angka di belakang koma',
            }),
        pufa: z
            .number()
            .positive()
            .refine((val) => /^\d+(\.\d{1,2})?$/.test(val.toString()), {
                message:
                    'PUFA harus memiliki maksimal 1 atau 2 angka di belakang koma',
            }),
        natrium: z
            .number()
            .positive()
            .refine((val) => /^\d+(\.\d{1,2})?$/.test(val.toString()), {
                message:
                    'Natrium harus memiliki maksimal 2 angka di belakang koma',
            }),
        cholesterol: z
            .number()
            .positive()
            .refine((val) => /^\d+(\.\d{1,2})?$/.test(val.toString()), {
                message:
                    'Kolesterol harus memiliki maksimal 2 angka di belakang koma',
            }),
    });
