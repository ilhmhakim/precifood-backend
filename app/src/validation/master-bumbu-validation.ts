...
export class MasterBumbuValidation {
  static readonly CREATE: ZodType = z.object({
    name: z
      .string({ required_error: 'Nama bumbu harus diisi' })
      .trim()
    ...
    calory: z
      ...
      .refine((val) => /^\d+(\.\d{1,20})?$/.test(val.toString()), {
        message: 'Kalori harus memiliki maksimal 20 angka di belakang koma',
      }),
    protein: z
      .number({ required_error: 'Protein harus diisi' })
      .min(0, 'Protein tidak boleh negatif')
      .refine((val) => /^\d+(\.\d{1,20})?$/.test(val.toString()), {
        message: 'Protein harus memiliki maksimal 20 angka di belakang koma',
      }),
    fat: z
      .number({ required_error: 'Lemak harus diisi' })
      .min(0, 'Lemak tidak boleh negatif')
      .refine((val) => /^\d+(\.\d{1,20})?$/.test(val.toString()), {
        message: 'Lemak harus memiliki maksimal 20 angka di belakang koma',
      }),
    carbohydrate: z
      ...
      .refine((val) => /^\d+(\.\d{1,20})?$/.test(val.toString()), {
        message:
          'Karbohidrat harus memiliki maksimal 20 angka di belakang koma',
      }),
    fiber: z
      .number({ required_error: 'Serat harus diisi' })
      .min(0, 'Serat tidak boleh negatif')
      .refine((val) => /^\d+(\.\d{1,20})?$/.test(val.toString()), {
        message: 'Serat harus memiliki maksimal 20 angka di belakang koma',
      }),
    natrium: z
      .number({ required_error: 'Natrium harus diisi' })
      .min(0, 'Natrium tidak boleh negatif')
      .refine((val) => /^\d+(\.\d{1,20})?$/.test(val.toString()), {
        message: 'Natrium harus memiliki maksimal 20 angka di belakang koma',
      }),
    cholesterol: z
      .number({ required_error: 'Kolesterol harus diisi' })
      .min(0, 'Kolesterol tidak boleh negatif')
      .refine((val) => /^\d+(\.\d{1,20})?$/.test(val.toString()), {
        message: 'Kolesterol harus memiliki maksimal 20 angka di belakang koma',
      }),
    sfa: z
      .number({ required_error: 'SFA harus diisi' })
      .min(0, 'SFA tidak boleh negatif')
      .refine((val) => /^\d+(\.\d{1,20})?$/.test(val.toString()), {
        message: 'SFA harus memiliki maksimal 20 angka di belakang koma',
      }),
    mufa: z
      .number({ required_error: 'MUFA harus diisi' })
      .min(0, 'MUFA tidak boleh negatif')
      .refine((val) => /^\d+(\.\d{1,20})?$/.test(val.toString()), {
        message: 'MUFA harus memiliki maksimal 20 angka di belakang koma',
      }),
    pufa: z
      .number({ required_error: 'PUFA harus diisi' })
      .min(0, 'PUFA tidak boleh negatif')
      .refine((val) => /^\d+(\.\d{1,20})?$/.test(val.toString()), {
        message: 'PUFA harus memiliki maksimal 20 angka di belakang koma',
      }),
  });
