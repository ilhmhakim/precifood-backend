...
export class RecommendationService {
    ...
    static async checkRecommendationForSpecificRestaurantandConsumer(
        restaurantId: string,
        consumerId: string
    ) {
    ...
    static async getRecommendation(request: GetRecommendationRequest) {
        const requestRecommendation = Validation.validate(
            RecommendationValidation.GETRECOMMENDATION,
            request
        );
        const recommendation =
            await this.checkRecommendationForSpecificRestaurantandConsumer(
                requestRecommendation.restaurant_id,
                requestRecommendation.consumer_id
            );
        const status = await this.checkGenerateStatus(
            requestRecommendation.consumer_id
        );
    ...
    static async getRecommendationListDetail(
        request: GetRecommendationListDetailRequest
        ...
        const recommendationList = await this.checkRecommendationDetail(
            requestRecommendation.recommendation_id,
            requestRecommendation.consumer_id
        );

        // Mengembalikan response dalam bentuk yang diinginkan
        return toGetRecommendationDetail(
            recommendationList,
            recommendationList.NutritionSummary!,
            recommendationList.RecommendationListDetail
        );
    }
    ...
    static async getRecommendationFromModel(
        ...
        const recommendationRequest = Validation.validate(
            RecommendationValidation.GENERATERECOMMENDATION,
            request
        );
        const status = await this.checkGenerateStatus(
        ...
        try {
            ...
            await UserService.checkRestaurant(
            ...
            if (response.status !== 200) {
                const error = new Error(
                    'Terjadi kegagalan pada generate model. Silahkan generate ulang!'
                );
            ...
            try {
                ...
                if (error instanceof Error) {
                    ...
                    await prismaClient.consumer.update({
                        ...
                        data: {
                            generator_error: `Error pada rekomendasi rank ${i + 1}: ${error.message}`,
                        },
                    });
                } else {
                    // Tangani jika error bukan instance dari Error
                    console.error(
                        `Unknown error creating recommendation at rank ${i + 1}:`,
                        error
                    );
                }
            }
    ...
    static async createRecommendation(
        ...
        const menuOne = await MenuService.checkMenuExist(
            recommendationDetail.makanan_pokok,
            restaurantId
        );
        const menuTwo = await MenuService.checkMenuExist(
            recommendationDetail.lauk_pauk,
            restaurantId
        );
        const menuThree = await MenuService.checkMenuExist(
            recommendationDetail.sayuran,
            restaurantId
        );
        const menuFour = await MenuService.checkMenuExist(
            recommendationDetail.minuman,
            restaurantId
        );
