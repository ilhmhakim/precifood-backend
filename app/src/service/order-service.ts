import {
    ...
    toOrderDetailResponse, UpdateOrderRequest
...
export class OrderService {
    static async checkOrder(consumer_id: string, order_id: number) {
        ...
        if (!order) {
            throw new ResponseError(404, `Order dengan id ${order_id} tidak ditemukan`);
        }
    ...
    static async createOrder(request: CreateOrderRequest): Promise<CreateOrderResponse> {
        ...
        const recommendationList = await RecommendationService.checkRecommendationDetail(createOrderRequest.recommendation_id, createOrderRequest.consumer_id);
    ...
    static async getOrderDetail(request: GetOrderDetailRequest): Promise<OrderResponse> {
        ...
        const order = await this.checkOrder(requestOrderDetail.consumer_id, requestOrderDetail.order_id);

        return toOrderDetailResponse(order, order.OrderDetail);
    }

    static async updateOrderStatus(request: UpdateOrderRequest) {
        const requestOrderStatusUpdate = Validation.validate(OrderValidation.GETORDERDETAIL, request);

        const order = await this.checkOrder(requestOrderStatusUpdate.consumer_id, requestOrderStatusUpdate.order_id);

        if (order.status == "Confirmed") {
            throw new ResponseError(403, 'Order yang telah terkonfirmasi tidak dapat diupdate lagi');
        }
    ...
    static async cancelOrder(request: CancelOrderRequest) {
        const requestCancelOrder = Validation.validate(OrderValidation.GETORDERDETAIL, request);

        const order = await this.checkOrder(requestCancelOrder.consumer_id, requestCancelOrder.order_id);

        // Cek status order
        if (order.status === "Confirmed") {
            throw new ResponseError(403, 'Order yang telah terkonfirmasi tidak dapat dicancel');
        }
        ...
        if (timeDifference > oneHourInMs) {
            throw new ResponseError(403, 'Order tidak dapat dicancel karena sudah lebih dari 1 jam sejak pemesanan');
        }
