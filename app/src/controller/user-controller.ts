...
export class UserController {
    static async registerConsumer(
        ...
        try {
            const request: CreateConsumerRequest =
                req.body as CreateConsumerRequest;
    ...
    static async registerRestaurant(
        req: Request,
        res: Response,
        next: NextFunction
    ) {
    ...
    static async getProfileConsumer(
        ...
        try {
            if (req.user.role == 'Konsumen') {
                const response = await UserService.getProfileConsumer(
                    req.user!
                );
                res.status(200).json({
                    message: 'Success!',
            ...
            if (req.user.role == 'Admin') {
                const request: GetUserProfileRequest =
                    req.body as GetUserProfileRequest;
    ...
    static async getProfileRestaurant(
        ...
        try {
            if (req.user.role == 'Restoran') {
                const response = await UserService.getProfileRestaurant(
                    req.user!
                );
                res.status(200).json({
                    message: 'Success!',
            ...
            if (req.user.role == 'Admin' || req.user.role == 'Konsumen') {
                ...
                const response =
                    await UserService.getProfileRestaurant(request);
                res.status(200).json({
                    message: 'Success!',
                    data: response,
                });
    ...
    static async getAllUserConsumer(
        req: UserRequest,
        res: Response,
        next: NextFunction
    ) {
    ...
    static async getAllUserRestaurant(
        req: UserRequest,
        res: Response,
        next: NextFunction
    ) {
    ...
    static async updateConsumer(
        ...
        try {
            const request: UpdateConsumerRequest =
                req.body as UpdateConsumerRequest;
