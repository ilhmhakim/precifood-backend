...
describe('seed', () => {
  it('should seeds', async () => {
    const response = await supertest(web).post('/api/seeds');
...
describe('Registrasi Konsumen (POST /api/signup/consumer)', () => {
  ...
  it('(+) Menerima registrasi konsumen jika seluruh data benar', async () => {
    const response = await supertest(web).post('/api/signup/consumer').send({
      name: 'Fix',
...
describe('Registrasi Restoran (POST /api/signup/restaurant)', () => {
  ...
  it('should sign up a new restaurant successfully', async () => {
    ...
    const response = await supertest(web).post('/api/signup/restaurant').send({
      name: 'Restoran Test',
      ...
      city: 'Bogor',
      address_detail:
        'Depan Pintu Tol Sentul Selatan-2 The Grand, Jl. Tol Lkr. Luar Bogor',
...
describe('Registrasi Admin (POST /api/signup/admin)', () => {
  ...
  it('should sign up admin successfully', async () => {
    const response = await supertest(web).post('/api/signup/admin').send({
...
describe('Melihat Profile / Detail Konsumen (GET)', () => {
  ...
  it('should return profile of a consumer when authenticated', async () => {
    const response = await supertest(web)
      ...
      .set(
        'Authorization',
        'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IkMtMDE5MmE3ZjgtMDhhNS03MzM2LWI5NDMtNjc1NDIxOGNlNTI4IiwiZW1haWwiOiJtYWxpa2FAZ21haWwuY29tIiwicm9sZSI6IktvbnN1bWVuIiwiaXNzIjoiTm9kZS1BdXRoIiwiaWF0IjoxNzI5Mzk1MTk4LCJleHAiOjE3Mjk1Njc5OTh9.QywCU8le0gHfct8TySSHAQe_EpQluM_6tZzfMu8Jpd0'
      );
  ...
  it('should return profile of a consumer when authenticated as admin', async () => {
    const response = await supertest(web)
      ...
      .set(
        'Authorization',
        'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IkMtMDE5MmU2YTYtYzY0ZS03OTljLTg5NTktZDZlNmE5NGIxMDUyIiwiZW1haWwiOiJhZGFtQGdtYWlsLmNvbSIsInJvbGUiOiJLb25zdW1lbiIsImlhdCI6MTczMDQ0NzQyNywiZXhwIjoxNzMwNjIwMjI3fQ.bRAiY4j7I7-MCKgQTErfIrjKIm6YRK5hbc-r5jbG744'
      );
...
describe('Melihat Profile / Detail Restoran (GET)', () => {
  ...
  it('(+) Melihat profile/detail restoran dari akun restoran', async () => {
    const response = await supertest(web)
      ...
      .set(
        'Authorization',
        'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IlItMDE5MjljY2ItYWM3NC03NTU5LWIyZjktNTBiMmM2ZjhjMTQ3IiwiZW1haWwiOiJ0ZXN0cmVzdGF1cmFudDJAZ21haWwuY29tIiwicm9sZSI6IlJlc3RvcmFuIiwiaXNzIjoiTm9kZS1BdXRoIiwiaWF0IjoxNzI5MjA3NDcwLCJleHAiOjE3MjkyMjE4NzB9.bwotfFBgvfwgVJq9KOqlqs4LRBhXru9oank9XSn1Kgs'
  ...
  it('(+) Melihat detail/profile restoran sebagai admin', async () => {
    const response = await supertest(web)
      ...
      .set(
        'Authorization',
        'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IkEtMDE5Mjk5ODUtY2FiNS03NzdmLWJjMmUtNDg1NTkyMjdiNDY2IiwiZW1haWwiOiJ0ZXN0YWRtaW5AZ21haWwuY29tIiwicm9sZSI6IkFkbWluIiwiaXNzIjoiTm9kZS1BdXRoIiwiaWF0IjoxNzI5MjA0NDgyLCJleHAiOjE3MjkyMTg4ODJ9.68wUwZTSEpK1wJpOdz982670OnJbhi8NE9_ITZh9vj0'
      );
...
describe('Mendapatkan seluruh list pengguna konsumen sebagai admin (GET)', () => {
  it('(+) (Admin) Mengembalikan data konsumen berupa id, nama, dan email', async () => {
    const response = await supertest(web)
      ...
      .set(
        'Authorization',
        'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IkEtMDE5Mjk5ODUtY2FiNS03NzdmLWJjMmUtNDg1NTkyMjdiNDY2IiwiZW1haWwiOiJ0ZXN0YWRtaW5AZ21haWwuY29tIiwicm9sZSI6IkFkbWluIiwiaXNzIjoiTm9kZS1BdXRoIiwiaWF0IjoxNzI5MjM4ODg0LCJleHAiOjE3MjkyODIwODR9.lnCUkRTEN15jXan9e1CYFzuM59PumPPo8ycPIx2BLH8'
      );
